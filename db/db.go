...
import (
	...
	mysqlMigrator "github.com/xinzf/kit/db/migrator/mysql"
	pstgreMigrator "github.com/xinzf/kit/db/migrator/postgresql"
...
func Migrator(tx *gorm.DB, schema ...string) migrator.Migrator {
	if tx.Name() == "mysql" {
		return mysqlMigrator.New(tx)
	} else if tx.Name() == "postgres" {
		return pstgreMigrator.New(tx, schema[0])
	} else {
